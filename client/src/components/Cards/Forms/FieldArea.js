...
class fieldArea extends Component {
  state = {
    showError: false,
    listItems: [uniqueId('listarea-')],
    checkedItems: [uniqueId('checkarea-')],
    loading: false
  };
  ...
  FieldComponent = ({ input }) => {
    return (
      <TextareaAutosize
        {...input}
        className="form-control mt-2 mb-1 z-depth-1-half"
        rows={4}
      />
    );
  };
  ...
  ListComponent = ({ input, meta }) => {
    return (
      <TextareaAutosize
        {...input}
        onBlur={() => this.onListChange(input, meta, 'list')}
        type="text"
        style={{ borderWidth: '0 0 1.2px 0' }}
  ...
  CheckComponent = ({ input, meta }) => {
    return (
      <input
        {...input}
        onClick={() => this.timedOut(input, meta)}
        className="cr-icon"
  ...
  onListChange = (input, meta, type) => {
    if (type === 'checkbox' && this.props.card) {
      if (
        !this.props.field.values[
          this.state.listItems[this.state.listItems.length - 1]
        ]
      ) {
        this.props.onChange(
          this.props.card._id,
    ...
    if (type === 'list' && meta.dirty === true && this.props.card) {
      ...
      const checkedField = this.state.checkedItems[
        this.state.listItems.indexOf(input.name)
      ];
  ...
  groupListComponent = (name, checkedName) => {
    return (
      <div className="input-group mt-3 mb-0 border-0">
        ...
        <Field name={name} type="text" component={this.ListComponent} />
      </div>
    );
  };
  ...
  render() {
    if (this.props.showText || this.props.type === 'text') {
      ...
      return (
        <div
          className="z-depth-1-half pt-1 pb-1 mt-2 mb-2"
          style={{ backgroundColor: 'white' }}
        >
          {this.renderList()}
          {this.state.loading && (
            <span
              className="spinner-border spinner-border-sm float-right mr-2 mt-4"
              role="status"
              aria-hidden="true"
            />
          )}
          {this.state.showError && (
            <p className="ml-5 text-danger">Please enter a value first!</p>
          )}
          <h5
            onMouseDown={() =>
              this.setState({
            ...
            className="ml-2 mt-3 createcard text-primary"
          >
            + Add List Item
          </h5>
        </div>
