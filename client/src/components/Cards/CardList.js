...
class CardList extends Component {
  ...
  renderCards() {
    ...
    const content = this.props.order.map((cardId, index) => {
      ...
      const lastItem =
        card.item.length > 0
          ? card.item[card.item.length - 1]
          : { _id: '1', text: 'notEmpty' };
      const isDroppable =
        lastItem.text === '' || !lastItem.hasOwnProperty('_id');
  ...
  onDragEnd = result => {
    ...
    if (start === finish) {
      newSourceItems.splice(
        destination.index,
        0,
        sourceCard.item[source.index]
      );
...
export default connect(
  mapStateToProps,
  { fetchCards, deleteCard, fetchBoard, dragCard, changeOrder }
)(CardList);
