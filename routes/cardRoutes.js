...
module.exports = app => {
  app.post('/api/cards', requireLogin, async (req, res) => {
    ...
    const listValues = Object.values(field).filter(
      value => typeof value === 'string'
    );
    const checkedNames = Object.keys(field).filter(name =>
      /checkarea/.test(name)
    );
    const checked = Object.values(field).filter(
  ...
  app.delete(
    ...
    async (req, res) => {
      ...
      await Card.find({ _id: req.params.id })
        .deleteOne()
        .exec();
      res.send('/');
    }
